...
interface CustomerAddressesSectionProps {
  ...
  /** Called whenever the address list changes (e.g. to refresh the customer's
   *  default address for POS delivery prefill). */
  onChanged?: () => void;
}

/**
 * Reusable address list + inline form. Renders as a section (no modal wrapper),
 * so it can be embedded inside a customer sidebar (page or POS) or a sheet.
 */
export function CustomerAddressesSection({ customerId, onChanged }: CustomerAddressesSectionProps) {
  const { t } = useI18n();
  ...
  const [form, setForm] = useState({
    label: '', street: '', number: '', complement: '', reference: '', neighborhood_id: '', is_default: false,
  });
  ...
  const openEdit = (a: CustomerAddress) => {
    setForm({
      label: a.label || '', street: a.street || '', number: a.number || '', complement: a.complement || '',
      reference: a.reference || '', neighborhood_id: a.neighborhood_id ? String(a.neighborhood_id) : '', is_default: a.is_default,
    });
    setEditing(a);
    setCreating(false);
  };

  const closeForm = () => { setEditing(null); setCreating(false); setSaving(false); };

  const save = async () => {
    if (!form.neighborhood_id) { toast.error(t('addresses.neighborhoodRequired')); return; }
    setSaving(true);
    const payload = {
      label: form.label.trim() || null, street: form.street.trim() || null, number: form.number.trim() || null,
      complement: form.complement.trim() || null, reference: form.reference.trim() || null,
      neighborhood_id: Number(form.neighborhood_id), is_default: form.is_default,
    };
    try {
      if (editing) await api.put(`/customer-addresses/${editing.id}`, payload);
      else await api.post('/customer-addresses', { ...payload, customer_id: customerId });
  ...
  const saveQuickNeighborhood = async () => {
    ...
    try {
      ...
      setForm((f) => ({ ...f, neighborhood_id: String(nb.id) }));
      setQuickNeighborhood({ open: false, name: '', fee: '0' });
    } catch (error: unknown) {
      const e = error as { response?: { data?: { error?: string } } };
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold uppercase tracking-wide text-gray-500">{t('addresses.title')}</h3>
        <Button type="button" variant="ghost" size="sm" onClick={openCreate} disabled={formOpen} className="gap-1">
      ...
      {loading ? (
        ...
        <p className="rounded-lg border border-dashed border-gray-200 px-4 py-6 text-center text-sm text-gray-400">{t('addresses.empty')}</p>
      ) : (
        <div className="space-y-2">
          {addresses.map((a) => (
